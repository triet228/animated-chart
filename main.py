...
# Convert dates to Matplotlib numbers for smooth interpolation
...
ax.xaxis.set_major_locator(mdates.YearLocator(2)) # Show tick every 2 years
...
# 4. Rendering
pbar = tqdm(total=TOTAL_FRAMES, desc="Rendering Smooth Battle")
def update(frame):
    pbar.update(1)
    for i, ticker in enumerate(tickers):
        lines[i].set_data(dates_interp[:frame], value_data[ticker][:frame])
    
    current_dt = mdates.num2date(dates_interp[frame])
